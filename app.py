...
# page config
apptitle = 'Vechicle Coupon'
st.set_page_config(page_title=apptitle, page_icon=':eyeglasses:', layout='wide')

# Create the Titles and Image
st.title("Vechicle Coupon Recommendation")
...
if st.button("Predict"):
   ...
   if result == 0:
      st.markdown("Awww you're :blue[not] eligible for the :red[coupon]")
   else:
      st.markdown("Congrates you're :blue[eigible] for the :green[coupon]")
   #st.text(result[0])
